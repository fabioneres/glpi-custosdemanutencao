(function() {
         ...
         if (dropdownType) {
            ...
            options.ajax = {
               ...
               data: function(params) {
                  ...
                  if (liveDropdownType === 'material') {
                     request.price_type = getMaterialPriceType(select[0]);
                  }
                  return request;
               },
   ...
   function getMaterialPriceType(select) {
      var form = select && select.closest ? select.closest('form') : null;
      var priceType = form ? form.querySelector('[name="price_type"]') : null;
      return priceType && priceType.value ? priceType.value : 'sinapi';
   }

   function rebuildAsyncDropdown(select) {
      if (!select) {
         return;
      }
      ...
      if (typeof jQuery !== 'undefined' && jQuery.fn.select2) {
         ...
         jQuery(select).removeData('maintenancecosts-ready');
      }
   ...
   function resetMaterialDependentFields(form) {
      if (!form) {
         return;
      }
      ...
      if (unitPrice) {
         unitPrice.value = formatDecimal(0);
   ...
   function updateMaterialSource(form, clearSelection) {
      if (!form) {
         return;
      }
      ...
      material.dataset.priceType = getMaterialPriceType(material);
      if (!clearSelection) {
         return;
      }

      rebuildAsyncDropdown(material);
      initPluginDropdowns(form);
      resetMaterialDependentFields(form);
   }
   ...
   function isContractOriginSelected(originSelect) {
      if (!originSelect) {
         return false;
      }

      var option = originSelect.options[originSelect.selectedIndex];
      if (!option) {
         return false;
      }

      if (option.dataset && option.dataset.isContract === '1') {
         return true;
      }

      return normalizeText(option.textContent || option.innerText || '') === 'contrato';
   }

   function updateContractRow(form) {
      if (!form) {
         return;
      }

      var row = form.querySelector('[data-maintenancecosts-contract-row]');
      var origin = form.querySelector('[name="plugin_maintenancecosts_materialorigins_id"]');
      if (!row || !origin) {
         return;
      }

      var shouldShow = isContractOriginSelected(origin);
      row.style.display = shouldShow ? '' : 'none';

      if (!shouldShow) {
         var contract = form.querySelector('[name="contracts_id"]');
         if (contract) {
            if (typeof jQuery !== 'undefined' && jQuery.fn.select2) {
               jQuery(contract).val(null).trigger('change');
            } else {
               contract.value = '0';
            }
         }
   ...
   function initOriginContractToggle(root) {
      (root || document).querySelectorAll('select[name="plugin_maintenancecosts_materialorigins_id"]').forEach(function(select) {
         if (select.dataset.maintenancecostsOriginToggleReady) {
            return;
         }
         select.dataset.maintenancecostsOriginToggleReady = '1';
