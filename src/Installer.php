...
class Installer
{
   ...
   private static function ensureSchema(): void
   {
      ...
      if ($DB->tableExists(Material::getTable())) {
         ...
         self::ensureField($migration, Material::getTable(), 'has_current_sinapi', 'tinyint NOT NULL DEFAULT 0');
         ...
         $migration->addKey(Material::getTable(), 'has_current_sinapi', 'idx_current_sinapi');
      }
   ...
   private static function syncStoredCostCenterNames(): void
   {
      /** @var DBmysql $DB */
      global $DB;

      foreach ([CostCenter::class, CostCenterLegacy::class] as $itemtype) {
         ...
         foreach ($DB->request(['FROM' => $table]) as $row) {
            $storedName = $itemtype::computeStoredName($row);
            if ($storedName === trim((string) ($row['name'] ?? ''))) {
               continue;
            }
